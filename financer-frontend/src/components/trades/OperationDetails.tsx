...
import { Swords, ShieldCheck, CheckCircle2, Plus, Trash2, Trophy, Star, Dices, Globe, Target, Zap, Flame, Shield, CircleDollarSign, Landmark} from 'lucide-react';
...
// ==========================================
// DICIONÁRIO ALINHADO COM A ENTIDADE JAVA
// ==========================================
const MapaIcones: Record<string, any> = {
  Swords, ShieldCheck, Trophy, Flame, Target, Star, Dices, Globe, Zap, Shield, CircleDollarSign, Landmark
};

// Puxa exatamente o campo 'cor' da sua Entidade
const getCorCasa = (casaDB: any) => casaDB?.cor || '#FFFFFF';

// Puxa exatamente o campo 'iconeId' da sua Entidade e traduz para Lucide
const getComponenteIcone = (casaDB: any) => {
  const nomeDoIcone = casaDB?.iconeId || 'Trophy'; // O SEGREDO ESTAVA AQUI!
...
export function ModalDetalhesOperacao({ trade, isOpen, onClose, onSave, onResolve }: ModalDetalhesOperacaoProps) {
  ...
  // Busca e guarda as casas do Java
  useEffect(() => {
    fetch('http://localhost:8080/api/casas')
      .then(res => res.json())
      .then(dados => {
        setCasasOficiaisDB(dados);
        console.log("Casas carregadas do DB:", dados); // Dica: Olhe o console (F12) para confirmar se os dados chegaram!
      })
  ...
  const handleAddNovaEntrada = () => {
    const novaEntrada: EntradaTrade = {
      id: `temp-${Date.now()}`,
      tipo: 'protecao',
      casa: '',
      odd: '' as any,
    ...
    setEntradasEditadas([...entradasEditadas, novaEntrada]);
  };

  const handleRemoveEntrada = (id: string) => {
    setEntradasEditadas(prev => prev.filter(ent => ent.id !== id));
  };
  ...
  const handleSalvar = () => {
    ...
    const entradasProntasParaOJava = entradasEditadas.map(ent => {
      ...
      return {
        ...ent,
        casaAposta: casaEncontradaNoBanco ? { id: casaEncontradaNoBanco.id } : undefined
      };
    });

    const casaInvalida = entradasProntasParaOJava.some(ent => !ent.casaAposta);
    if (casaInvalida) {
      alert("Erro: Uma das casas não foi encontrada no banco de dados.");
      return;
    }
  ...
  const lucroAoVivo = isEditing 
    ? calcularLucroProjetado(entradasParaCalculo, trade.mercado, 0) 
    : trade.lucro;
  ...
  const handleConfirmarResolucao = () => {
    if (totalRecebidoDinamico <= 0) {
      alert("Insira o valor recebido nas casas vencedoras.");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      ...
      <div className="w-full max-w-lg bg-[#18181b] border border-border/80 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 relative z-50">
              ...
              <div key={entrada.id} className={`flex flex-col p-4 border rounded-xl relative ${corTema} animate-in fade-in`}>
                ...
                <div className="flex items-center justify-between mt-1 mb-2">
                  {isEditing ? (
                    <select
                      value={entrada.casa}
                      onChange={(e) => handleAtualizarEntradaStr(entrada.id, 'casa', e.target.value)}
                      className="w-[70%] py-1.5 px-2 text-sm font-bold text-white bg-[#18181b] border border-border/50 rounded-lg outline-none focus:border-brand"
                    >
                      <option value="">Selecione a Casa...</option>
                      {casasOficiaisDB.map((c: any) => (
                  ...
                  {isEditing && !isPrincipal && (
                    <button onClick={() => handleRemoveEntrada(entrada.id)} className="p-1.5 text-danger hover:bg-danger/10 rounded-lg transition-colors border border-transparent hover:border-danger/30" title="Excluir Entrada">
          ...
          <div className="flex flex-col p-5 border-t border-brand/30 bg-brand/5 animate-in slide-in-from-bottom-2 gap-4">
            ...
            <div className="flex flex-col gap-3">
              {casasVencedoras.map((casa, index) => {
                // Acha a casa no DB garantindo que maiúsculas/minúsculas não quebrem o código
                const casaDetalhes = casasOficiaisDB.find(
                  c => c.nome.toLowerCase() === casa.casaNome.toLowerCase()
                );
                ...
                return (
                  <div key={casa.id} className="flex items-end gap-2 relative z-[65]">
                    <div className="relative flex-1">
                      ...
                      <InputSelect 
                        label={index === 0 ? "Casa de Aposta" : ""} 
                        icon={IconeAtivo}
                        iconHexColor={corAtiva}
                        placeholder="Selecione..."
                        value={casa.casaNome}
                        onClick={() => setDropdownAbertoId(dropdownAbertoId === casa.id ? null : casa.id)}
                      />

                      {dropdownAbertoId === casa.id && (
                        <>
                          <div className="fixed inset-0 z-[60]" onClick={() => setDropdownAbertoId(null)} />

                          <div className="absolute left-0 right-0 top-[100%] mt-2 max-h-56 overflow-y-auto bg-[#131316] border border-border/80 rounded-xl shadow-2xl z-[70] custom-scrollbar p-2 flex flex-col gap-1">
                            {casasOficiaisDB.map((casaDB: any) => {
                              ...
                              return (
                                <div 
                                  ...
                                  onClick={() => {
                                    updateCasaVencedora(casa.id, 'casaNome', casaDB.nome);
                                    setDropdownAbertoId(null);
                                  }}
                                >
                                  <div 
                                    className="flex items-center justify-center w-10 h-10 rounded-xl border shadow-sm relative overflow-hidden"
                                    style={{ borderColor: corHex, color: corHex }}
                                  >
